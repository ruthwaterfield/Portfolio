...
echo 'hi';
/**
 * addContentToPage updates content item given the id of the item
 *
 ...
 */
function editContentWithId(string $textLabel, string $textContent, string $imageLabel, string $imageUrl, int $id) : bool {
    echo 'here2';
    $result = 0;
    try {
        ...
        if(imageShouldBeAdded($imageLabel, $imageUrl)) {
            echo 'herewithimage';
            $sql = "UPDATE `Content` SET `textLabel` = :textLabel, `textContent` = :textContent, `imageLabel` = :imageLabel, `imageUrl` = :imageUrl WHERE `id` = :id;";
 ...
 */
function imageShouldBeIncluded(string $imageLabel, string $imageUrl) : bool {
    $result = 1;
